...
- (void)dealloc {
    [_images_ release], [self setImages_:nil];
   [super dealloc];
}
...
//      self.images_ = [[NSMutableArray alloc] initWithObjects:
...
- (void)imageAtIndex:(NSInteger)index photoView:(KTPhotoView *)photoView {
   NSDictionary *imageVo = [self.images_ objectAtIndex:index];
   NSString *url = [imageVo objectForKey:@"imageurl"];
   [photoView setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"photoDefault.png"]];
}

- (void)thumbImageAtIndex:(NSInteger)index thumbView:(KTThumbView *)thumbView {
//   NSArray *imageUrls = [self.images_ objectAtIndex:index];
